...
composants forment un test. Un \inenglish{assertion checker} est donc capable de
...
À présent, illustrons comment utiliser le RAC de Praspel où le SUT est la
méthode \code{f} de la classe \code{C} de la
figure~\ref{figure:tools:dispatching} avec un seul paramètre \code{\$i}~:
%
\begin{pre}
...
\end{pre}
%
La variable \code{\$rac} contient une instance d'un \inenglish{Runtime Assertion
Checker} qui travaille avec un modèle objet dans la variable \code{\$model}
passée en premier argument et un SUT passé en second argument. Ensuite, grâce à
la méthode \code{setData}, nous fixons les données des arguments du SUT, à
savoir que son paramètre \code{\$i} aura comme valeur l'entier \code{13}. Enfin,
le SUT sera exécuté et le contrat vérifié grâce à l'appel de la méthode
\code{evaluate} sur le RAC. Dans ce cas, la variable \code{\$verdict} contiendra
\code{true} car la précondition (\code{\arequires i: 7..42}) est bien respectée.
%
\begin{tikzannotation}
