...
\begin{tikzpicture}[myoverlay]
    \draw [mybrace]
        (a.north -| boxtoolsdispatching.east)
        -- node [mybracetext] {Praspel} 
        (b.south -| boxtoolsdispatching.east);
    \draw [mybrace]
        (c.north -| boxtoolsdispatching.east)
        -- node [mybracetext] {SUT}
        (d.south -| boxtoolsdispatching.east);
\end{tikzpicture}
...
En s'appuyant sur la figure~\ref{figure:tools:dispatching}, nous rappelons qu'à
partir d'une classe, les commentaires qui annotent les méthodes contiennent des
contrats écrits en Praspel. Les contrats seront donc extraits de ces
commentaires pour être envoyés dans les outils de Praspel. Les méthodes vont
constituées le SUT et sera utilisé lors de l'exécution des tests. Les prochaines
figurent vont reprendre ces «~mots-clés~» à plusieurs reprises pour illustrer le
flux de ces différents composants.
