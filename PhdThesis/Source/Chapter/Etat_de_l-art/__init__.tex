...
critères ne sont pas exhaustives. En effet, ce n'est parce qu'un test à activer
tous les chemins que toutes les erreurs ont été évitées. De plus, des erreurs
peuvent se cacher dans l'environnement d'exécution du programme. Notons que si
nous travaillons en boîte-noire, ces critères ne sont pas appliquables, et nous
pouvons uniquement nous référer aux postconditions des contrats.
...
sur la résolution de contraintes (comme TestEra, Korat ou UDITA), ou l'exécution
symbolique de code (comme Symstra, EXE, DART, KLEE ou CUTE).
...
nous positionner sur le code source du programme. Cette technique demande peu ou
pas d'efforts à l'utilisateur. Des travaux ont été réalisés dans ce sens pour
...
\paragraph{ARTOO} \acite{CiupaLOM08} \\
...
code~\acite{MillerM63, Myers04} maximale. Dans le second cas, une «~bonne~»
