...
Notons que pour un langage a typage dynamique et faible, même un contrat très
simple qui spécifie le typage des données, peut être utile, comme démontré dans
le travail sur les types graduels~\acite{SiekT07}.
...
\inenglish{grey-box} ou \inenglish{black-box}).
...
plupart des outils des CBT travaillent en \inenglish{black-box}\footnote{TODO~:
préciser ou enlever.}.

Mais «~un test montre la présence, jamais l'absence d'une erreur~» (Edsger
W.~Dijkstra dans~\acitep{Buxton70}). Et un bon test est un test qui échoue. Dans
...
l'exécution symbolique de code (comme Symstra, CUTE, DART ou Java PathFinder).
...
\paragraph{EXE} \acite{CadarGPDE08}

\paragraph{KLEE} \acite{CadarDE08}
