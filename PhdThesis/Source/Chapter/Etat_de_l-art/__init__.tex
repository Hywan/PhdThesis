...
Les chaînes de caractères sont intensivement utilisées dans des programmes Web.
Leurs formes sont très différentes, cela va d'une adresse email à des langages
comme XML ou JSON, en passant par des requêtes SQL etc. La façon la plus
répandue dans l'industrie pour représenter une donnée textuelle est d'utiliser
une expression régulière ou une grammaire algébrique, soit d'une manière
générale, à partir d'une grammaire~\acite{Chomsky56}. Une grammaire est exprimée
à partir d'un {\strong langage de description} qui déclare des {\strong lexèmes}
(unité lexicale atomique d'un langage) et des {\strong règles}, exprimant
l'enchaînement des lexèmes les uns par rapport aux autres.
...
\paragraph{Yagg} \acite{CoppitL05}

\paragraph{Geno} \acite{LammelS06}

\paragraph{YouGen} \acite{HoffmanLSW11}

\paragraph{Apollo}

\paragraph{@Flajolet} \acite{FlajoletZC94}

\paragraph{@Howden} \acite{Howden86}

+/- conclure avec @Hennessy~\acite{HennessyP08}.

\subsection{Génération de tableaux}

\subsection{Génération d'objets}
