...
du \inenglish{dynamic binding} (aussi appelé \inenglish{late binding},
représenté en général par la variable \code{this}), c'est~à~dire que la
...
et \code{ensure}. Le contenu de ces constructions est exprimé avec le langage
Eiffel lui-même. Le langage introduit également le mot-clé \code{assert} pour
...
\code{requires} et \code{ensures} pour introduire les préconditions et les
postconditions. Une postcondition exceptionnelle est introduite par le mot-clé
\code{signals}. Les invariants sont introduits par le mot-clé \code{invariant}.
JML propose la clause \code{also} qui permet de raffiner, par un nouveau
...
construction \aold{i}, présente uniquement dans une postcondition, qui fait
...
\code{\bslash{}forall} ($\forall$) et \code{\bslash{}exists} ($\exists$).
