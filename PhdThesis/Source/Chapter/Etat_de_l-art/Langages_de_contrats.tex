...
au paradigme orienté objet, le support des exceptions et une documentation
automatique. L'aspect objet apporte quelques spécificités, comme les invariants
de classes, la gestion de l'héritage multiple, la résolution de conflit avec
l'héritage multiple, comme le renommage de certains attributs ou méthodes, sans
oublier la gestion du \inenglish{dynamic binding} (aussi appelé \inenglish{late
binding}, représenté en général par la variable \code{this}), c'est~à~dire que
la résolution des appels se fait à l'exécution et non pas à la compilation.
