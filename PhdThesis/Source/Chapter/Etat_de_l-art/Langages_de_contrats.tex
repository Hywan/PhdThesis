...
représenté en général par la variable \code{this} qui représente l'instance de
l'objet), c'est~à~dire que la résolution des appels se fait à l'exécution et non
pas à la compilation. Les exceptions nécessitent également de définir dans quels
contextes elles peuvent être levées et dans quel état se trouve l'objet après
qu'une exception ait été levée. Cette information est d'autant plus importante
que le langage Eiffel permet de ré-exécuter la routine (méthode) qui a déclenché
