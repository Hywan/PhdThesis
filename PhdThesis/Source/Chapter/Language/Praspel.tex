...
construction \apred{p} supporte la validation mais pas la génération. Toutes les
variables présentes dans $p$ doivent avoir un domaine réaliste associée (en
dehors de la construction \apred{p}). Le prédicat
\code{\bslash{}pred('\$this->getUsage() + \$file->getSize()
...
tout comme les valeurs. Les clés peuvent être auto-incrémentées, en ajoutant 1
à la dernière clé entière, à partir de 0. La longueur (ou la taille) d'un
tableau est son nombre de paires. Un tableau n'a pas de longueur prédéfinie,
mais sa longueur (stockée en interne par le moteur PHP) peut être connue grâce à
la fonction PHP \code{count()}. Un tableau n'a également pas de profondeur
