...
sont conçus pour spécifier des domaines de données {\strong pertinents} pour un
...
son parent. Par exemple, le domaine réaliste \code{Color} (qui représente des
couleurs) hérite de \code{String} (qui représente des chaînes de caractères), et
\code{Boundinteger} (qui représente des intervalles d'entiers) hérite de
\code{Integer} (qui représente l'ensemble des entiers de la machine). Tous les
domaines réalistes doivent être enfants de la classe \code{Realdom}, qui assure
...
paramètres~: \code{lower} et \code{upper} en \circled{4}, qui ont pour domaine
réaliste \code{Constinteger}, avec respectivement en valeur par défaut
\code{PHP\_INT\_MIN} et \code{PHP\_INT\_MAX} qui sont des constantes de PHP. Si
aucune valeur par défaut n'est précisée, alors le paramètre est {\strong
obligatoire}, sinon il est {\strong optionnel}.
...
Alors, nous pouvons écrire \code{str\-ing('a', 'z', 4..12)} sans produire
...
interfaces}. Elles aident à caractériser les domaines réalistes. Les interfaces
