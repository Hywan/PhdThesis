...
en problème de satisfaction de contraintes (\inenglish{Constraint Satisfcation
...
\Function{$\akw{AC3}$}{}
  ...
  \While{$\avar{queue} \neq \emptyset$}
      ...
      \If{$\acall{AC3\_Arc\_Reduce}(x, y)$}
          ...
          \ForAll{$z \in \acall{Neighbours}(x)$}
...
\Function{$\akw{AC3\_Arc\_Reduce}$}{}
    ...
    \ForAll{$d_x \in \avar{Domains}(x)$}

        \If{$\akw{there~is~no} d_y \in \avar{D}(y) \akw{s.t} d_y.\acall{reduce}(d_y)$}
...
%Le principe d'un tel algorithme est de transformer un CSP en un graphe où chaque
%nœud représente une variable et les arcs représentent les con\-train\-tes, ce
%afin de propager et de vérifier la consistence des contraintes. L'algorithme,
%présenté dans la figure~\ref{figure:data:ac3}, va sélectionner un arc (une
%contrainte) à traiter~: les variables seront réduites si possible. Si une des
%deux variables est liée à une autre variable à travers une contrainte, alors un
%nouvel arc sera créé jusqu'à ce qu'aucune réduction ou propagation des
%contraintes ne puissent être appliquées, c'est~à~dire jusqu'à ce qu'il n'y ait
%plus d'arcs à traiter (la variable $\avar{queue}$ représente cet ensemble
%d'arcs). La particularité de cette variante est que nous utilisons les domaines
%réalistes pour représenter l'ensemble des valeurs possibles pour une variable.
%Nous avons cinq sortes de domaines réalistes représentées par les interfaces
...
%une ou des méthodes de raffinement pour permettre la réduction du domaine. Cette
%méthode s'appelle $\akw{reduce}$ dans l'algorithme. Par exemple,
%\code{Nonconvex} attend une méthode \code{discredit} pour supprimer une valeur
%du domaine, \code{Interval} attend les méthodes \code{reduceRightTo} et
%\code{reduceLeftTo} pour réduire respectivement la borne droite et gauche etc.
%L'algorithme AC3 travaille sur des contraintes unaires et binaires,
%respectivement représentées par les variables $\avar{R_1}$ et $\avar{R_2}$ dans
%l'algorithme.
...
(de \code{a} jusqu'à \code{e}), alors nous enlevons 0 et 10 de $S$ pour obtenir
...
révisons alors \code{natural(0, 1)} pour qu'il ne produise que 5 données au
...
exemple, \code{Nonconvex} attend une méthode \code{discredit} pour supprimer une
valeur du domaine, \code{Interval} attend les méthodes \code{reduceRightTo} et
\code{reduceLeftTo} etc. La consistence vérifie également qu'il n'y a pas de
