...
La propagation des contraintes utilise un algorithme AC-3~\acite{Mackworth77},
...
nœud représente une variable, et les arcs représentent les contraintes.
...
S$ (que nous avons déduite). Nous révisons les valeurs de $S$~: la variable $Y =
\code{string('a', 'e', 1)}$ peut contenir 1 à 5 données maximum (de \code{a}
jusqu'à \code{e}), alors nous enlevons 1 et 10 de $S$ pour obtenir $S =
[\code{1..5}]$. Nous ajoutons l'arc $(X, Y)$ traité juste avant. Cette fois-ci,
nous pouvons réviser $Y$ avec la contrainte $\mathrm{card}(X) =
