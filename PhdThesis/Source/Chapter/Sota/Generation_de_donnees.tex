...
par rapport à une spécification. Nous pouvons citer toutefois le prototype
{\scshape sante} qui fait à une approche intéressante entre le
\inenglish{concolic} et les contrats.

\paragraph{Sante} Les outils de \inenglish{concolic} sont efficaces pour valider
des programmes mais passent difficilement à l'échelle sur des programmes de
taille importante. Le prototype~\inenglish{Static ANalysis and
...
code C (comme JML est compilé en Java avec JMLUnit), et le programme est
instrumenté pour les exécuter. Ainsi, PathCrawler prendra en considération les
contrats ACSL lors de la génération de tests. Toutefois, {\scshape sante} ne
supporte qu'un petit sous-ensemble du langage ACSL~: les assertions prévenant
les divisions par zéro, les dépassements de tableaux et des débordements
arithmétiques.
