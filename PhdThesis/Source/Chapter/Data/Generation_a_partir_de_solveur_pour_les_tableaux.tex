...
cardinalité de $X$ est égale à la cardinalité de $X_1$, qui est inférieure ou
égale à la cardinalité de \code{natural(0, 1)}, qui vaut $+\infty$. Nous n'avons
rien à réviser, $S$ reste alors inchangée. $X$ a un arc vers $Y$, donc ajoutons
la paire $(Y, X)$ dans la variable $\avar{queue}$.

\item Nous sélectionnons et retirons la deuxième paire $(S, S)$. Nous avons la
con\-train\-te unaire (ne portant que sur une seule variable) $S \geq 0$. Nous
n'avons rien à réviser. $S$ a un arc vers $X$, donc nous ajoutons la paire $(X,
S)$.
...
\subseteq X$. Nous vérifions $\code{0} \in \code{natural(0, 1)}$. Tout est
...
$Y$. La cardinalité de $Y$ est égale à la cardinalité de $Y_1$, qui est
inférieure ou égale à la cardinalité de \code{string('a', 'e', 1)}, qui vaut $5$
car ce domaine réaliste peut produire les données \code{a} jusqu'à \code{e}.
Nous savons aussi que le tableau contiendra au moins une paire. Ainsi,
$\mcard(X) \in [1; 5]$. Dans la variable $\avar{queue}$, nous devrions ajouter
la paire $(X, S)$, mais elle est déjà présente.
...
enlever les valeurs \code{0} et \code{10}. $L_1$ est un domaine réaliste
