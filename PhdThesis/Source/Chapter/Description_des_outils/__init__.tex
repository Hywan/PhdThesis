...
l'architecture de Praspel, ses différentes bibliothèques et composants, comment
ils s'articulent entre eux etc. Ensuite, la
partie~\ref{subsection:tools:interpretation} présente comment interpréter
Praspel, suivie de la partie~\ref{subsection:tools:compilation} qui présente
comment compiler Praspel. Enfin, la partie~\ref{subsection:tools:evaluation}
présente comment est évalué Praspel.
...
\drawfig{10cm}{!}{node distance=3cm}{
    ...
    \node (model)    [rectangle, minimum height=3cm                   ] { Object model };
    \node (language) [left of=model                                   ] { Language };
    \node (php)      [right of=model                                  ] { PHP };
    \node (verdict)  [rectangle, below of=model, minimum height=1cm,
                      node distance=4cm                               ] { Verdict };
    \node (callable) [left of=verdict                                 ] { Callable };
...
spécification (un contrat) est objet etc. Tous ces objets sont imbriqués et
...
\end{pre}
%
qui permet de construire le modèle objet contenu dans \code{\$model}. Cet
exemple nous permet également d'avoir un aperçu de l'API du modèle objet. Elle
se veut le plus simple et compréhensible possible.
...
la spécification sur le système sous test. Soit des données nécessaires à
l'évaluation lui sont fournies, soit il va les générer de lui-même à partir de
la spécification.
...
Considérons par exemple la méthode \code{store} et son contrat dans la
...
Voici comment utiliser le RAC de Praspel~:
%
\begin{pre}
function f ( \$i ) \{ \} \\
 \\
...
pré-condition (\code{\arequires i: 7..42}) est bien respectée. Si nous voulons
...
pouvoir appeler les méthodes \code{sample} sur ces derniers sans soucis)~:
%
\begin{pre}
...
Praspel est un langage de spécification destiné à PHP et qui est construit de
façon modulaire et extensible. Afin de faciliter son développement et sa
maintenance, nous avons choisi de nous appuyer sur le projet Hoa.
...
De plus, le fait que Hoa se veut être un pont entre le monde de la recherche et
...
Nous outils prennent la forme de bibliothèques dans Hoa, à savoir
\code{Hoa\bslash Realdom} pour les domaines réalistes, \code{Hoa\bslash
Praspel} pour le langage Praspel, \code{Hoa\bslash Compiler} pour le compilateur
et \code{Hoa\bslash Regex} pour les expressions régulières.

\subsubsection{\code{Hoa\bslash Realdom}}

La bibliothèque \code{Hoa\bslash Realdom} représente la bibliothèque standard
des domaines. Ils sont implémentés comme présenté dans la
partie~\ref{section:language:realdoms} page~\ref{section:language:realdoms}, à
savoir que chaque domaine réaliste est représenté par une classe.
