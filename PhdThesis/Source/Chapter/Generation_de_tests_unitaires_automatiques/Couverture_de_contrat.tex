...
syntaxique \grule{exception-identifier}, pour $1 \leq i \leq n$. Nous
...
\grule{declaration}, \grule{predicate} et \grule{constraint}. Nous transformons
seulement les deux premières sortes d'expressions car les expressions de sorte
\grule{constraint} ne sont pas pertinentes pour la couverture~: elles
représentent un seul comportement (par exemple, \grule{constraint} et
\grule{declaration} apparaissent toujours ensemble) et sont toujours évaluées.
...
La première partie de la figure~\ref{figure:test:expression_graph} présente
graphiquement le graphe associé à
l'exemple~\ref{example:test:expression_graph1}.
...
La deuxième partie de la figure~\ref{figure:test:expression_graph} présente
graphiquement le graphe associé à
l'exemple~\ref{example:test:expression_graph2}.
...
\end{pre}
%
où $R$ et $E$ sont des expressions de sorte \grule{expression} respectivement
...
\code{\athrowable $T_{C_\alpha}$ with $T_{E_\alpha}$}. Dans la figure, les
