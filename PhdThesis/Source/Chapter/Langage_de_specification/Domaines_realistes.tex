...
parent. Par exemple, le domaine réaliste \code{Color} (qui représente une
couleur) hérite de \code{String} (qui représente une chaîne de caractères), et
\code{Boundinteger} (qui représnte un intervalle d'entiers) hérite de
\code{Integer} (qui représente un entier). Tous les domaines réalistes doivent
être enfant de la classe \code{Realdom}.
...
dont les code-points des caractères sont entre \code{0x61} et \code{0x7a}.
