...
de contrats en PHP (voir la partie~\ref{subsection:sota:cbt}). Ce langage
...
Le reste de la partie décrit les parties de la grammaire de Praspel, en forme
normale.
...
\code{or}. Dans la figure~\ref{figure:language:short_contract}, la variable
...
la vérification (générabilité). Le prédicat \apred{\code{'\$this->getUsage() +
\$file->getSize() > static::SIZE'}} dans la
...
\code{SIZE}, alors dans \code{Filesystem}, \code{self::SIZE} sera équivalent à
\code{Filesystem::SIZE} alors que \code{static::SIZE} sera équivalent à
\code{BigFilesystem::SIZE}.
