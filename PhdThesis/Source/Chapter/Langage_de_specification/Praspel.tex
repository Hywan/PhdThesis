...
vérification, \ie par les caractéristiques de prédicabilité et de générabilité
des domaines réalistes. Chaque concept introduit dans le langage doit supporter
ces deux aspects. Mais ils arrivent que certaines choses ne puissent pas être
...
équivalent à \code{BigFilesystem::SIZE}. Ce mécanisme est appelé le
\inenglish{Late Static Binding}\footnote{Voir \url{http://php.net/lsb}.}.

\subsection{Description de tableaux}
\label{subsection:language:array}

La description de tableaux est une partie très importante de Praspel, contrôlant
