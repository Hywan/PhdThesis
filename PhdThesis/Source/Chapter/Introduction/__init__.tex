...
Dans ce mémoire, nous proposons un nouveau langage de contrat. Pour élaborer un
tel langage, nous avons trois axes de réflexion~: le langage lui-même (sa
syntaxe et sa sémantique), les algorithmes de générations de données, et enfin
les algorithmes de générations de tests.

\subsection{Un langage de contrats pour PHP}

Nous proposons un langage de contrat. Ce contrat peut être utilisé de deux
façons différentes~: soit il sera évalué lors de l'exécution du programme afin
de vérifier qu'aucune spécification n'est violée, soit nous pouvons à partir
d'un contrat générer automatiquement plusieurs tests unitaires. Cette seconde
façon est traitée dans les contributions suivantes. Cette contribution
s'intéresse au langage en lui-même, à savoir sa syntaxe et sa sémantique. Nous
voyons aussi comment évaluer un contrat pour qu'il vérifie des données à
l'exécution. Cette contribution a été publiée dans~\acitei{EnderlinDGO11}.

\subsection{Des générateurs de données}

Nous avons dit qu'un contrat pouvait être utilisé pour générer
automatiquement des tests unitaires. Un test est constitué de trois choses~: un
SUT, des données de test et un oracle. Cette contribution s'intéresse à générer
des données de test. Ces données sont spécifiées par le contrat et représentent
au mieux les données manipulées par le programme. Nous devons alors être
capables de générer toutes sortes de données~: des booléens, des entiers, des
réels, des chaînes de caractères, des tableaux, des objets etc. Cette
contribution a donné lieux à deux publications~: \acitei{EnderlinDGB12} et
\acitei{EnderlinGB13}.

\subsection{Un générateur automatique de tests unitaires}
...
devons nous assurer que tous ces comportements sont testées. Nous allons alors
générer autant de tests unitaires que nécessaire pour couvrir tous ces
comportements. Les données nécessaires pour exécuter ces tests seront générées
