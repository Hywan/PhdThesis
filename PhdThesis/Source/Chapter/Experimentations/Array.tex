\section{Étude de cas~: solveur sur les tableaux}
\label{section:experimentation:solver}

Cette partie présente l'expérimentation que nous avons menée dans
l'article~\acitei{EnderlinGB13}. Dans cette partie, nous utilions le mot {\em
...
\inenglish{backtracks} dans le solveur et le nombre de systèmes rejetés. Quand
un système est rejeté, ses \inenglish{backtracks} ne sont pas comptabilisés.
...
\inenglish{backtracks}. Pour éviter des piques dans les résultats, nous
...
faible nombre de \inenglish{backtracks}. $n = 20$ représente approximativement
